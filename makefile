...
help:
	...
	@echo '  make indent <files> : indents a files with uncrustify               '
...
# Indentation related
###########################################################################
# indent target depends on source files only
indent: $(addsuffix .indent, $(filter %.cpp %.c %.h %.hpp, $(MAKECMDGOALS)))

#recipes for indented files, see uncrustify man page and uncrustify.cfg for reference
%.indent : %
	@echo '  INDENT $<'
	@uncrustify -q -f $< -c uncrustify.cfg -o $@
	@mv -f $@ $<
...
###########################################################################
.PHONY : all help size gccversion build elf hex bin lss sym clean createdirs indent
.DEFAULT_GOAL := all
